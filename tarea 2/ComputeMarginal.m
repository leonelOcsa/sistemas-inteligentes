...
jointDist = ComputeJointDistribution(F); %aplicamos un joint de todos los factores 
ObsEv = ObserveEvidence(jointDist, E); %y luego calculamos la evidencia observada sobre el factor resultante en base a E
vm = setdiff(ObsEv.var, V); %calculamos la diferencia para obtener las variables a marginalizar
FacMar = FactorMarginalization(ObsEv, vm) %obtenemos un factor resultado parcial marginalizado
totalSum = sum(FacMar.val) %calculamos la suma total de valores 
%y finalmente normalizamos
FacMar.val = (FacMar.val)./totalSum;  
