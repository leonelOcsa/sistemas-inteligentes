...
def parte3_2(corpus_file1, corpus_file2):
                            ...
                            if not S[k] in dictionary:
                                ...
                                if wordS.isupper(): #comprobamos si todas las letras son mayusculas
                                    ...
                                    if isNumeric > 0: #comprobamos si es numerico
                                        eKey = "_NUMERIC_|" + tags_k[v] 
                                    else:
                                        lenw = len(lword)
                                        if isLastCapital == 1 and lword[lenw-1].isupper(): #comprbamos si solo el ultimo caracter es Mayuscula
                                            eKey = "_LAST_CAPITAL_|" + tags_k[v] 
                                        else: 
...
if __name__ == "__main__":
    ...
    parte3_2(input1, input2)
    #python parte3_2.py gene.out2.counts gene.dev -> para llamar a la funcion y generar gene_dev.p3.out

    #y luego verificamos el score llamando a
